// # Pense no seguinte problema, um mercado recebe uma quantidade de dados de um mercado
// # Recebemos um Json com os produtos, preços, quantidade em estoque e o tipo de venda (Unid, Kg ou ambas[Kg/Unid])

// # Implementação: Aplicações da Programação Funcional

// # Conforme estudamos nas últimas aulas, o Cálculo Lambda criado por Alonzo Church serve como base para a Programação Funcional que conhecemos hoje. 
// # Com base nestes conceitos funcionais, diversas linguagens de programação adotaram algumas características funcionais em complemento à sua estrutura sintática usual.

// # Um dos exemplos clássicos de aplicação das estruturas funcionais em linguagens de programação modernas são os manipuladores de listas map, filter e reduce. 
// # Em resumo as funções citadas são descritas a seguir:
...
// # Escolha a linguagem de sua preferência. Não é preciso utilizar obrigatoriamente uma linguagem de programação funcional. Basta uma linguagem que dê suporte ao que 
// # foi pedido e caso não tenha, você pode implementar estes conceitos. Linguagens atuais como Java8+, C#, Javascript, Python, Go ou PHP possuem total suporte à funções 
// # lambda e ao map, filter e reduce.
// # Escolha o problema que quiser resolver. A ideia é mostrar um pequeno problema e sua solução utilizando programação imperativa (procedural ou orientada a objetos). 
// # A partir disso, apresentar uma solução utilizando conceitos funcionais.

// # Entrega:
// # Trabalho Individual
// # Código fonte no Github ou Gist (se for um script com apenas um arquivo)
// # Um pequeno relatório (comentário no código fonte) explicando o problema e qual a vantagem de aplicar a implementação funcional na sua resolução.
...
function main(){
    ...
    console.log(arqTeste[0]);
    // console.log(arqTabela);
